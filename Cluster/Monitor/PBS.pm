...
=head2
	...
	PACKAGE		Engine::Cluster::Monitor::PBS
	
    VERSION:        0.02

    PURPOSE
  
        MONITOR JOBS RUN ON A PBS (PORTABLE BATCH SCHEDULER) SYSTEM
...
#### Str
has 'qstat'		=> ( isa => 'Str', is => 'rw', default => "qstat" );
...
sub submitJob ( $args ) {
	...
	my ($job_id) = $output =~ /$self->jobidregex()/;
	use re 'eval';	# EVALUATE AS REGEX
...
sub jobLines {
	...
	my $result = $self->repeatTries($command, $self->errorregex(), $sleep, $tries);
...
sub repeatTries ( $command, $error_regex, $sleep, $tries ) {
	...
	while ( $error_message and $tries )
	{
		...
		use re 'eval';	# EVALUATE AS REGEX
		$error_message = $result =~ /$self->errorregex()/;
...
{"username":"syoung","sessionid":"1234567890.1234.123","project":"Project1","workflow":"Workflow1","mode":"executeWorkflow","number":1}
...
sub checkJob ( $job_id ) {
	...
	$checkjob = $self->checkjob() if not defined $checkjob;
	...
	#### SLEEP BETWEEN TRIES
	my $sleep		=	$self->get_sleep();	
	$sleep = $self->sleep() if not defined $sleep;
	$self->logDebug("sleep", $sleep);
...
sub status ( $args ) {
    ...
    my $total = $self->table()->db()->query($query);
    ...
    #### GET FIELDS
    my $fields = $self->table()->db()->fields('monitor');
    $self->logDebug("Fields: @$fields");

    #### RUN QUERY
    my $jobs = $self->table()->db()->querytwoDarray($query);
...
sub update {
    ...
    my $pids = $self->table()->db()->queryarray($query);
    foreach my $pid ( @$pids )
    {        
        my $qstat_result = '';
...
sub create_db {
    ...
    my $success = $self->table()->db()->create_table($table, $sqlfile);
