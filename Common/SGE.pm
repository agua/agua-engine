...
#### QUEUES
method createQueuefile ( $queue, $queuefile, $parameters ) {
...
method setQueuefileContents ( $queue, $parameters ) {
...
#### STARCLUSTER ONLY
method _addQueue ( $queue, $queuefile, $parameters )  {
	$self->logDebug("");
...
method _removeQueue ( $queue, $queuefile, $running ) {
	$self->logDebug("queue", $queue);
...
method modifyQueue ( $queue, $queuefile ) {
...
method queueExists ( $queue, $qmasterport, $execdport ) {	
	$self->logDebug("queue", $queue);
...
method deleteQueueJobs ( $sgebin, $queue ) {
...
method setPEFileContents ( $pe, $slots ) {	
	# OPTIONAL ENTRIES:
  # slots              AUTO
...
method addPEToQueue ( $pe, $queue, $queuefile ) {
	$self->logDebug("pe", $pe);
	$self->logDebug("queue", $queue);
	$self->logDebug("queuefile", $queuefile);
...
method getQueuefile ( $queue ) {
	...
	my $fileroot = $self->util()->getFileroot($adminuser);
...
#### WORKFLOW ONLY
method checkSge ( $username, $cluster ) {
	...
	#### SET MASTER OPS Util::Ssh OBJECT
...
=cut
method checkHeadnodeSge ( $execdport ) {
...
=cut
method restartHeadnodeSge ( $execdport ) {
	$self->logDebug("Engine::Workflow::restartHeadnodeSge(execdport)");
	$self->logDebug("execdport", $execdport);
...
method stopHeadnodeSge ( $execdport ) {

	$self->head()->ops()->stopSgeProcess($execdport);
}

method startHeadnodeSge ( $execdport ) {
	$self->logDebug("");
	my $sgeBinCommand = $self->sgeBinCommand("head");
...
method setHeadnodeSubmit ( $cluster, $qmasterport, $execdport, $oldname, $newname ) {
	$self->logDebug("cluster : $cluster ");
...
method setHeadnodeHostname ( $hostname ) {
...
method setHeadnodeHostnamefile ( $hostname ) {
...
#### QCONF COMMANDS
method setSchedulerKey ( $key, $value ) {
...
method deleteAdminHost ( $hostname ) {
	$self->logDebug("hostname", $hostname);
	
	$self->sgeQconfCommand("-dh $hostname");
}
	
method deleteSubmitHost ( $hostname ) {
	$self->logDebug("hostname", $hostname);

	$self->sgeQconfCommand("-ds $hostname");
}

method deleteExecutionHost ( $hostname ) {
	$self->logDebug("hostname", $hostname);
...
method removeFromAllHosts ( $hostname ) {
	$self->logDebug("hostname", $hostname);
...
method sgeQconfCommand ( $options ) {
	my $sgebin = $self->sgeBinCommand("head");
	$self->logDebug("sgebin", $sgebin);
...
method checkMasterSge ( $qmasterport, $execdport ) {
	$self->logDebug("qmasterport", $qmasterport);
	$self->logDebug("execdport", $execdport);
...
method setMasterOpsSsh ( $mastername ) {
	$self->logDebug("mastername", $mastername);
...
#### PERMANENTLY SET MASTER HOSTNAME
method setMasterHostname ( $hostname ) {
...
### PERMANENTLY SET MASTER HOSTNAME
method setMasterHostnamefile ( $hostname ) {
...
method restartMasterSge ( $qmasterport, $execdport ) {
...
### Add new master IP (long dns name) to submit hosts and admin hosts lists
method setSgeSubmitHosts ( $cluster, $qmasterport, $execdport, $oldname, $newname ) {
	$self->logDebug("cluster : $cluster ");
	$self->logDebug("qmasterport : $qmasterport ");
	$self->logDebug("execdport : $execdport ");
	$self->logDebug("oldname : $oldname ");
...
method updateHeadnodeEtcExports ( $oldmasterip, $newmasterip ) {
...
method updateMasterEtcFstab ( $oldheaddnsname, $newheaddnsname ) {
	my $file = "/etc/fstab";
	my $inserts = [];
	my $removes = [];
...
method resetMaster {
	...
	my $clusterhash = $self->table()->db()->queryhash($query);
...
#### REBOOT AN INSTANCE
method rebootInstance ( $instance ) {
...
method setHeadnodeMasterInfo ( $cluster ) {
...
method _setHeadnodeMasterInfo ( $cluster, $internalfqdn, $internalip, $instanceid, $externalip, $externalfqdn ) {
...
method updateHeadnodeInSge ( $oldheadname, $newheadname ) {
...
method update ( $username, $cluster, $qmasterport, $execdport, $oldheadname, $newheadname, $oldmastername, $newmastername, $newmasterip ) {
...
=cut
method updateMasterInSge ( $cluster, $qmasterport, $execdport, $oldname, $newname, $newip ) {
	$self->logDebug("cluster", $cluster);
	$self->logDebug("qmasterport", $qmasterport);
	$self->logDebug("execdport", $execdport);
	$self->logDebug("oldname", $oldname);
...
WHERE qmasterport > $minmaster};
		...
		if ( not defined $qmasterport )
		{
			...
			my $count = $self->table()->db()->query($query);
			if ( $count == 0 )
			{
			...
			else
			{
				...
				$self->logDebug("$query");
				my $ports = $self->table()->db()->queryarray($query);
