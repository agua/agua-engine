...
class Test::Engine::Workflow::Unit with (Test::Table,
	Test::Common) extends Engine::Workflow {
...
method BUILD ($hash) {
	print "Test::Engine::Workflow::Unit::BUILD\n";
	
	$self->initialise($hash);
}

method initialise ($hash) {
	print "Test::Engine::Workflow::Unit::initialise\n";
	if ( $hash ) {
...
method setStarCluster {
	$self->logDebug("");
	my $starcluster = Test::StarCluster::Main->new({
...
#### RETURN INSTANCE OF StarCluster
    ...
    #### TEST StarCluster ISA Test::StarCluster::Main
    isa_ok($self->starcluster(), "Test::StarCluster::Main");
...
method testExecuteWorkflow {
	...
	*{ensureSgeRunning} = sub {
		...
		shift->logDebug("OVERRIDE Engine::Workflow::ensureSgeRunning");
	};

	#### OVERRIDE WORKFLOW createQueue
	*{createQueue} = sub {
		shift->logDebug("OVERRIDE Engine::Workflow::createQueue");
	};
